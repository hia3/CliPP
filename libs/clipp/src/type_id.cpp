...
#include <boost/clipp/detail/decorated_type_id.hpp>
#include <utility>
#include <vector>
#include <algorithm>
#include <memory>
...
#  ifdef BOOST_CLIPP_HAVE_GCC_CP_DEMANGLE
...
namespace detail
{
  BOOST_CLIPP_EXPORT_IMPORT char const* gcc_demangle(char const* mangled)
  {
      typedef std::vector<
...
BOOST_CLIPP_EXPORT_IMPORT std::ostream& operator<<(std::ostream& os, std::type_info const& x)
{
    return os << x.name();
}
...
namespace detail
{
  BOOST_CLIPP_EXPORT_IMPORT std::ostream& operator<<(std::ostream& os, detail::decorated_type_info const& x)
  {
      os << x.m_base_type;
