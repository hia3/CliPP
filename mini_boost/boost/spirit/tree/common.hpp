...
public:
    ...
    : len(x.len), trees(), val(impl::tree_match_attr<T>::get(x))
    {
...
struct common_tree_tree_policy
{
    ...
    template <typename Iterator1T, typename Iterator2T,typename ValueT>
        static node_t
        create_node(int /*length*/, 
                    Iterator1T const& first, Iterator2T const& last, 
                    ValueT const& val,
                    bool leaf_node,mpl::bool_<true>)
    {
    ...
    template <typename Iterator1T, typename Iterator2T,typename ValueT>
        static node_t
        create_node(int /*length*/, Iterator1T const& first, 
