...
namespace boost { namespace spirit {
    ...
    struct char_parser : public parser<DerivedT>
    {
        ...
        parse(ScannerT const& scan) const
        {
            ...
            iterator_t save_inner=scan.first;
            if (!scan.at_end())
            {
                value_t ch = *scan;
    ...
    public:
        ...
        chseq(IteratorT first, IteratorT last)
        : first_(first), last_(last) {}

        chseq(IteratorT first)
        : first_(first), last_(impl::get_last(first)) {}
        ...
        IteratorT first() const {return first_;}
        IteratorT last() const {return last_;}
    private:
    ...
    public:
        ...
        chseq<IteratorT> seq() const {return seq_;}
    private:

        chseq<IteratorT> seq_;
    };
